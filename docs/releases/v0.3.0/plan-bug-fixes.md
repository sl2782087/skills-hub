...
来源：https://github.com/qufei1993/skillverse/issues

---

## Bug 1：Git 安装时 skill 名称为 "skills" 导致路径重复（#28）

**Issue**: https://github.com/qufei1993/skillverse/issues/28
**严重程度**: P0
...
**Issue**: https://github.com/qufei1993/skillverse/issues/20
**严重程度**: P0
...
**Issue**: https://github.com/qufei1993/skillverse/issues/18 / https://github.com/qufei1993/skillverse/issues/8
**严重程度**: P1
**状态**: ✅ 已修复
...
**Issue**: https://github.com/qufei1993/skillverse/issues/12
**严重程度**: P1
...
**Issue**: https://github.com/qufei1993/skillverse/issues/19
**严重程度**: P2
**状态**: ✅ 已关闭（非 bug，预期行为）

### 调查结论

`~/.skillverse/` 是 Skillverse 的内部存储，外部工具不应直接写入。实际上 OpenCode 创建的 skill 会落在 `~/.config/opencode/skills/` 下（普通目录），不会进入 `~/.skillverse/`。用户误以为写入了 skillverse 目录。

如需将外部工具中新建的 skill 纳入 Skillverse 管理，可通过"导入已有 Skill"功能手动操作。

---

## Bug 7：不支持 .claude/skills/ 目录格式的仓库（#27）

**Issue**: https://github.com/qufei1993/skillverse/issues/27
**严重程度**: P1
**状态**: ✅ 已修复（PR #31 合并）

### 问题描述

使用 `.claude-plugin/plugin.json` + `.claude/skills/` 目录结构的仓库（如 `nextlevelbuilder/ui-ux-pro-max-skill`）无法被 Skillverse 识别和安装，因为扫描逻辑只查找 `SKILL.md` 文件。
...
**Issue**: https://github.com/qufei1993/skillverse/issues/29
**严重程度**: P2
